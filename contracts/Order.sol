...
//    Chainlink public chainlink;
    ...
    constructor(address _orderStore, address _foundStore) {
        orderStore = OrderStore(address(_orderStore));
        foundStore = FoundStore(address(_foundStore));
    ...
    */
    ...
    // TOOO: faltara saber de qui es la la pasta da cada un, aixo al tancar una ordre, es a dir al tancar un trade
...
*/
        ...
        foundStore.transferIn{value: msg.value}();
    ...
    function cancelOrders(uint256[] calldata orderIds, string[] calldata reasons) external {
        for (uint256 i = 0; i < orderIds.length; i++) {
    ...
    function _cancelOrder(uint256 orderId, string memory reason) internal {
        ...
        if (!order.isReduceOnly) {
            foundStore.transferOut(order.user, order.margin + order.fee);
            //foundStore.transferOut(order.asset, order.user, order.margin + order.fee);
        }
